...
function Chip({ href, children, expected }: { href: string; children: React.ReactNode; expected: Record<string, string> }) {
  ...
  return (
    <Link href={href} aria-current={active ? 'page' : undefined} className={cn(baseClass, active ? 'header-active-in border-primary bg-primary text-primary-foreground shadow-[0_5px_14px_hsl(var(--color-text)/.13)]' : 'border-line bg-surface/72 text-ink-muted hover:border-ink/35 hover:text-ink')}>
      {children}
    </Link>
  );
}
...
export function CatalogHeaderNav() {
  ...
  return (
    <>
      <Link href="/catalog" aria-current={catalogActive ? 'page' : undefined} className={cn('h-[42px] w-[182px] flex items-center justify-center rounded-full border text-[13px] font-semibold transition-[background-color,border-color,color,box-shadow,transform] duration-300 ease-out', catalogActive ? 'header-active-in border-primary bg-primary text-primary-foreground shadow-[0_5px_14px_hsl(var(--color-text)/.13)]' : 'border-line bg-surface/72 text-ink-muted hover:border-ink/35 hover:text-ink')}>Каталог</Link>
