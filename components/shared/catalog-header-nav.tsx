...
const baseClass = 'h-[38px] px-4 inline-flex items-center rounded-full border text-[13px] font-semibold transition-colors';
...
function Chip({ href, children, expected }: { href: string; children: React.ReactNode; expected: Record<string, string> }) {
  ...
  return (
    <Link href={href} aria-current={active ? 'page' : undefined} className={cn(baseClass, active ? 'border-primary bg-primary text-primary-foreground' : 'border-line bg-surface text-ink-muted hover:border-ink/35 hover:text-ink')}>
      {children}
    </Link>
  );
}
